...
function Check() {
    ...
    setTimeout(() => {
        ...
        sign = 1
        if (NoList[0] && pflag==0) { //有支持节点、且为策略组才操作
            console.log("开始排序")
            ReOrder(NoList)
            } else if (!OKList[0]){ //不支持
                content =pflag==1 ? `<p style="text-align: center; font-family: -apple-system; font-size: large; font-weight: thin"><br><b>😃 该节点支持 YouTube 去广告 </b><br><br>👇<br><br><font color=#FF5733>-------------------------<br><b>⟦ `+policy+` ⟧ </b><br>-------------------------</font>`: `<p style="text-align: center; font-family: -apple-system; font-size: large; font-weight: thin">` + "<br>❌  <b>⟦ "+policy+ " ⟧ </b>⚠️ 切换失败<br><br><b>该策略组内未找到送中节点" + "<br><br><font color=#FF5733>-----------------------------<br><b>检测详情请查看JS脚本记录</b><br>-----------------------------</font>"+`</p>`
                $done({"title":"YouTube Premium 检测&切换", "htmlMessage": content})
            } else if (NoList[0]){ //支持, 但为节点
                content =`<p style="text-align: center; font-family: -apple-system; font-size: large; font-weight: thin"><br><b> 🎉 该节点支持 YouTube 去广告 </b><br><br>👇<br><br><font color=#FF5733>-------------------------<br><b>⟦ `+policy+` ⟧ </b><br>-------------------------</font>`
                $done({"title":"YouTube Premium 检测&切换", "htmlMessage": content})
        }
    }, relay)
...
function ReOrder(cnt) {
    ...
    let Ping =resolve.ret[array[0]]
        ...
        $configuration.sendMessage(mes1).then(resolve => {
            if (resolve.error) {
                console.log(resolve.error);
                content =pflag==1 && array[0]? `<p style="text-align: center; font-family: -apple-system; font-size: large; font-weight: thin">` + "<br><b>⟦ "+policy+ " ⟧ </b><br><br>🎉 该节点支持 <b>YouTube 去广告</b>" + `</p>` : `<p style="text-align: center; font-family: -apple-system; font-size: large; font-weight: thin">` + "<br><b>⟦ "+policy+ " ⟧ </b><br><br>⚠️ 该节点不支持 <b>YouTube 去广告</b>" + `</p>`
                content = pflag!=0 && !array[0]? `<p style="text-align: center; font-family: -apple-system; font-size: large; font-weight: thin">` + "<br>❌  <b>⟦ "+policy+ " ⟧ </b>⚠️ 切换失败<br><br>该策略组内未找到支持 YouTube 去广告的节点" + "<br><br>-----------------------------<br><b><font color=#FF5733>检测详情请查看JS脚本记录</font></b><br>-----------------------------"+`</p>` : content
                $done({"title":"YouTube 检测&切换", "htmlMessage": content})
            }
            if (resolve.ret) {
                console.log("已经切换至支持 <b>YouTube 去广告</b> 的路线 ➟ "+array[0])
                if (cronsign == "Y") { $notify("📺 YouTube Premium 定时检测&切换", "🎉 已经切换至YouTube 去广告的最优延迟线路👇", array[0] +"\n 👉 "+Ping)}
                content = `<p style="text-align: center; font-family: -apple-system; font-size: large; font-weight: thin">` + "<br><b>⟦ "+policy+ " ⟧ </b>已切换至支持<b>YouTube 去广告</b> 的路线中延迟最优节点<br><br> 👇<br><br> ⟦ "+array[0]+ " ⟧" + "<br><br><font color=#16A085>"+Ping+"</font><br>-----------------------------<br><b><font color=#FF5733>检测详情请查看JS脚本记录</font></b><br>-----------------------------"+`</p>`
                $done({"title":"YouTube 检测&切换", "htmlMessage": content })
            }
    }, reject => {
            $done();
